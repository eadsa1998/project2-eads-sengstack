...
DROP TABLE IF EXISTS tweet_info_load;
CREATE TABLE IF NOT EXISTS tweet_info_load (
...
tweet_ratio FLOAT,
PRIMARY KEY (entry_id), # Set primary key to entry_id
...
INSERT INTO tweet_info_load (tweet_id, user_id, created_at, collected_at, source, tweet, likes, retweet_count, total_popularity, candidate, tweet_ratio)
...
# Create the table for each specific tweet where tweet_id is the primary_key
...
SELECT entry_id, user_id, created_at, collected_at, source, tweet, likes, retweet_count, total_popularity, candidate, tweet_ratio
FROM election_tweets.tweet_info_load;
